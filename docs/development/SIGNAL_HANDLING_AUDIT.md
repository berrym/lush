...
Status: **audit + proposed model -- no code commitment.** S1-S8 are the
read-only current-state audit; S9 is the proposed lush signal model for
...
- **Few, centralized blocking boundaries.** Unlike bash's sprawling
  reader/executor, lush blocks in a countable set of places (S5.5). This is
...
|---|---|---|---|---|
| SIGINT | `sigint_handler` (forward to child / abort line) | `signals.c:225` | caught; re-prompt | **keep**, harden (S3.7) |
| SIGSEGV | `sigsegv_handler` (crash report -> abort) | `signals.c:226` -> `errors.c:75` | usually `SIG_DFL` | keep, but handler not async-safe (S3.7) |
...
| SIGCHLD | none (synchronous reap) | -- | handler + job-table reap | **research** (P4, S7) |
| SIGPIPE | none (default `SIG_DFL`) | -- | shell-specific; children `SIG_DFL` | **verify** -- no divergence found (G7) |
| SIGTSTP/SIGTTIN/SIGTTOU | `SIG_IGN` via raw `signal()` | `executor.c:16432-16434` | `SIG_IGN` in job-control shell | keep, but use `sigaction` (S3.2) |
| SIGWINCH | `handle_sigwinch` (flag) -- three installers | `terminal_unix_interface.c:203`, `base_terminal.c:562`, masked in `display_controller.c` | caught; redraw | **change** -- one owner (S3.6) |
| SIGCONT | `handle_sigcont` (re-arm termios) | `terminal_unix_interface.c` | caught / default | keep, harden termios (S3.6) |
...
(`4470`), subshell (`6936`), command-sub (`15739`), background x2
...
- **TCSAFLUSH wedge class** (S2) -- `TCSANOW` was applied only at
...
| G19 | No SIGCHLD reaper; `$!`/`wait -n`/job-status timing all synchronous | tree-wide | -- | **research -- P4, S7** |
...
hangs (those were TCSAFLUSH + the write-only flag, S1). Its real payoff is
...
By the CNS config-vs-code boundary (`CONFIG_NERVOUS_SYSTEM.md` S6):

- **Disposition is data.** Each signal's disposition is an enum cell
  `signal.<name>.disposition  in  {default, ignore, handle}`, layered
...
- **The trap table is data.** The signal->handler mapping is a keyed relation,
  the same category as keybindings and aliases (which S6 places in CREG).
- **The trap body is code.** The command string is a *declarative parameter*
  (the alias-value analog, S6 data) stored in the table so `trap -p` can print
...
P1 (S6) builds the front door + `run_pending_signals` + worker-block + unified
fork reset. P2 adds the CREG policy layer (S9.1) and wires trap dispatch to the
safe points, closing G1/G2/G8. P3 (#375) and P4 (SIGCHLD, deferred) are
unchanged. The event-system completion (S9.3) is sequenced with P1 (the reader
`pselect` + front door) but its full async-dispatch build is its own LLE-scope
workstream.

---

*End of audit. No source changed. S1-S8 are the current-state audit; S9 is the
proposed model for decision. Remediation is proposed in phases (S6) and
scheduled separately after the S9.5 decisions are settled.*
