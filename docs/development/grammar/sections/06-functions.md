...
keyword_function_form
                     ...
                     ; (§6) parens are optional iff next token is "{"

parameters           = parameter { "," parameter }

parameter            = NAME [ "=" default_value ]
                     ; (§6) lush extension; not POSIX, not bash

default_value        = WORD | STRING | EXPANDABLE_STRING

brace_group_body     = "{" command_list (§1) "}"
                     ; (§6) body MUST be a brace group; subshell, if, etc. NOT accepted
```
