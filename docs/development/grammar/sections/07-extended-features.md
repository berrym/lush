...
arithmetic_command = "((" arith_token_seq "))"
                   ; (§7) inner content is collected verbatim and reconstructed
...
array_literal      = "(" { array_element [ separator ] } ")"
                   ; (§7) ONLY recognized in assignment context
...
extended_test      = "[[" test_token_seq "]]"
                   ; (§7) inner content reconstructed as a string in
...
```
process_substitution_in  = "<(" command_list (§1) ")"
process_substitution_out = ">(" command_list (§1) ")"
```
