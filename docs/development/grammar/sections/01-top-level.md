...
command_list       = logical_expression { separator logical_expression } [ separator ]
                   ; (S1) parse_command_list -- does NOT consume control-structure
...
logical_expression = pipeline { ( "&&" | "||" ) separator pipeline }
                   ; (S1) left-associative; `&&` and `||` share precedence

pipeline           = [ "!" wsp ] simple_command_or_compound
                     { ( "|" | "|&" ) [ newline ] simple_command_or_compound }
                     [ "&" ]
                   ; (S1) `!` is tokenized as TOK_WORD with text=="!", length==1
                   ; "&" backgrounds the entire pipeline

command_body       = { separator } [ logical_expression { separator logical_expression } { separator } ]
                   ; (S1) parameterized by terminator token; stops without consuming it

if_body            = { separator } [ logical_expression { separator logical_expression } { separator } ]
                   ; (S1) commands wrapped in NODE_COMMAND_LIST; stops at
                   ;     TOK_ELSE, TOK_ELIF, or TOK_FI

simple_command_or_compound = simple_command (S2)
                           | brace_group (S3) | subshell (S3)
                           | if_statement (S3) | while_statement (S3)
                           | until_statement (S3) | for_statement (S4)
                           | select_statement (S4) | time_command (S4)
                           | coproc (S4) | anonymous_function (S4)
                           | case_statement (S5) | function_definition (S6)
                           | arithmetic_command (S7) | extended_test (S7)
```
...
**Keyword-mode flag.** The tokenizer carries `enable_keywords`. `TOK_DONE`, `TOK_FI`, `TOK_ELSE`, `TOK_ELIF` are only produced when this flag is set. Top-level body parsers rely on this -- if keyword mode were off, terminators would arrive as TOK_WORD and control structures would never close. See S9 for the full mode story.
