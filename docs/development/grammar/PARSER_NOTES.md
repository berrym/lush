...
- **EOF before delimiter**: the loop exits with all remaining input collected as the body, with **no error reported**. This is an open issue -- bash diagnoses it. See open questions S10.
...
3. **`while`/`until` conditions are not logical_expression.** See S4.2.
...
- **Differential testing**: every accepted/rejected input from the bash-compatible subset can be cross-checked with `bash -n`. Productions marked as lush-permissive (S7) are known divergence points and should be excluded from differential checks.
