...
/// Body width of a wrapped comment line, excluding the leading "# ".
#define CREG_COMMENT_WRAP_COL 76

/// True for ASCII whitespace -- the breakable separators between words. All
/// schema help/description text is ASCII, so this also guarantees a break never
/// lands inside a multibyte sequence.
static bool creg_is_ascii_space(char c) {
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f' ||
           c == '\v';
}

/// Write @p text as one or more "# "-prefixed comment lines, breaking at ASCII
/// whitespace so no line's body exceeds CREG_COMMENT_WRAP_COL columns (a single
/// over-long word is left intact rather than split). Any run of whitespace --
/// including a stray newline in a literal -- collapses to a single break, so
/// the output is always well-formed comment lines. A NULL or empty @p text
/// writes nothing.
static void write_comment_wrapped(FILE *file, const char *text) {
    if (!text || !text[0]) {
        return;
    }
    size_t col = 0;
    ...
    while (*p) {
        ...
        const char *end = p;
        while (*end && !creg_is_ascii_space(*end)) {
            end++;
        }
        size_t wordlen = (size_t)(end - p);
        if (line_open && col + 1 + wordlen > CREG_COMMENT_WRAP_COL) {
            fputc('\n', file);
            line_open = false;
        }
        if (!line_open) {
...
creg_result_t config_registry_save(const char *path) {
    ...
    fprintf(file, "# Generated by `config save`. Only settings changed from "
                  "their defaults are\n");
    fprintf(file, "# listed; each is documented with the same text the setup "
                  "wizard shows.\n");
    fprintf(file, "# Edit freely -- comment lines are ignored on load.\n\n");
    ...
    for (size_t i = 0; i < g_registry.section_count; i++) {
        ...
        for (size_t j = 0; j < reg->option_count; j++) {
            ...
            /// Write the section header before the first key; otherwise
            /// separate this stanza from the previous one with a blank line.
            if (!section_written) {
                fprintf(file, "[%s]\n", reg->section.name);
            ...
            /// Document the key above its value: the plain-language description
            /// when present, else the terse one-line help. Every persisted key
            /// has at least help, so a saved file is self-explanatory.
            if (opt->option_def) {
                write_comment_wrapped(file, opt->option_def->description
                                                ? opt->option_def->description
                                                : opt->option_def->help);
            }
