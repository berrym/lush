...
static lle_result_t rb_grow(render_buf_t *rb, size_t need,
    ...
    if (need > SIZE_MAX - 1 - rb->len) return LLE_ERROR_OUT_OF_MEMORY;
    size_t required = rb->len + need + 1;
    if (required <= rb->cap) return LLE_SUCCESS;

    size_t new_cap = rb->cap ? rb->cap : 32;
    while (new_cap < required) {
        if (new_cap > SIZE_MAX / 2) return LLE_ERROR_OUT_OF_MEMORY;
        new_cap *= 2;
    }
    char *new_buf = lle_pool_alloc(new_cap);
    if (!new_buf) return LLE_ERROR_OUT_OF_MEMORY;
...
lle_result_t lle_splicer_render_for_context(const char *text,
    ...
    switch (quote_state) {
        ...
        case LLE_QUOTE_ESCAPE_PENDING:
            ...
            return render_none(text, text_length, pool, out_rendered,
                               out_length);
        case LLE_QUOTE_DOUBLE:
            return render_double(text, text_length, pool, out_rendered,
                                 out_length);
        case LLE_QUOTE_BACKTICK:
            return render_backtick(text, text_length, pool, out_rendered,
                                   out_length);
        case LLE_QUOTE_SINGLE:
            return render_single(text, text_length, pool, out_rendered,
                                 out_length);
    }
    return LLE_ERROR_INVALID_PARAMETER;
}
...
char lle_splicer_close_char(lle_quote_state_t quote_state) {
    switch (quote_state) {
        case LLE_QUOTE_SINGLE:   return '\'';
        case LLE_QUOTE_DOUBLE:   return '"';
        case LLE_QUOTE_BACKTICK: return '`';
        case LLE_QUOTE_NONE:
        case LLE_QUOTE_ESCAPE_PENDING:
            return '\0';
    }
