    ...
    for (int i = opt_idx; i < argc; i++) {
        ...
        if (eq) {
            ...
            if (opt_nameref) {
                ...
                // Array literal: local arr=(a b c). The parser bundles
                // the (...) into one argv element, but bin_local was
                // historically treating it as a scalar so ${#arr[@]}
                // returned 0 instead of the element count. Walk the
                // value the same way bin_declare does for declare -a.
