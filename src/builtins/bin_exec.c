...
int bin_exec(int argc, char **argv) {
    ...
    /// The shell process is about to be replaced -- reset every disposition to
    /// default and clear the mask so the exec'd program (e.g. `exec tmux`)
    /// starts from a clean slate and is not left with the shell's ignored
    /// signals or the startup SIGHUP block masked in.
