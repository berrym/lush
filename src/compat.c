...
int compat_init(const char *data_dir) {
    ...
    if (xdg_data_home && xdg_data_home[0]) {
        ...
        if (home && home[0]) {
            snprintf(path_buf, sizeof(path_buf), "%s/%s/%s", home,
                     XDG_DATA_HOME_DEFAULT, COMPAT_SUBDIR);
            total_loaded += try_load_from_dir(path_buf);
        }
    }
    ...
    if (xdg_data_dirs && xdg_data_dirs[0]) {
        ...
        if (dirs_copy) {
            ...
            while (dir) {
                snprintf(path_buf, sizeof(path_buf), "%s/%s", dir,
                         COMPAT_SUBDIR);
                total_loaded += try_load_from_dir(path_buf);
...
size_t compat_get_entry_count(void) { return g_compat.entry_count; }
 ...
 *
 * Returns true if the behavior indicates the feature doesn't work in that
 * shell.
 */
...
static const char *get_behavior_for_target(const internal_entry_t *entry,
                                           const char *target) {
    if (!target || strcasecmp(target, "posix") == 0 ||
        strcasecmp(target, "sh") == 0) {
...
#define AST_FEATURE_MAP_SIZE                                                   \
    (sizeof(ast_feature_map) / sizeof(ast_feature_map[0]))
...
static size_t compat_check_ast_node(node_t *node, shell_mode_t target,
    ...
    while (child && found < max_results) {
        found =
            compat_check_ast_node(child, target, results, max_results, found);
...
static size_t compat_collect_ast_issues(node_t *node, shell_mode_t target,
                ...
                for (size_t j = 0; j < g_compat.entry_count; j++) {
                    if (g_compat.entries[j].feature &&
                        lle_unicode_strings_equal(
                            g_compat.entries[j].feature,
                            ast_feature_map[i].feature,
                            &LLE_UNICODE_COMPARE_DEFAULT)) {
                        issue->severity =
                            compat_severity_name(g_compat.entries[j].severity);
    ...
    while (child && found < max_issues) {
        found =
            compat_collect_ast_issues(child, target, issues, max_issues, found);
        child = child->next_sibling;
    }
...
void compat_set_strict(bool strict) { g_compat.strict_mode = strict; }

bool compat_is_strict(void) { return g_compat.strict_mode; }
