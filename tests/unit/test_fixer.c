...
TEST(fixer_load_string_null_content) {
    ...
    ASSERT_EQ(result, FIXER_ERR_IO,
              "load_string with NULL content should fail");
...
TEST(fixer_load_file_nonexistent) {
    ...
    ASSERT_EQ(result, FIXER_ERR_IO,
              "load_file with nonexistent path should fail");
...
TEST(fixer_count_null) {
    ...
    ASSERT_EQ(fixer_count_unsafe(NULL), 0,
              "count_unsafe(NULL) should return 0");
    ASSERT_EQ(fixer_count_manual(NULL), 0,
              "count_manual(NULL) should return 0");
}
...
TEST(fixer_apply_fixes_alloc) {
    ...
    fixer_result_t result =
        fixer_apply_fixes_alloc(&ctx, &opts, &output, &applied);
...
TEST(fixer_interactive_apply_none_accepted) {
    ...
    size_t applied = 0;
    fixer_result_t result =
        fixer_interactive_apply(&session, &output, &applied);
...
TEST(fixer_result_string_all) {
    ...
    ASSERT_STR_EQ(fixer_result_string(FIXER_ERR_PARSE), "Parse error",
                  "Parse string");
    ASSERT_STR_EQ(fixer_result_string(FIXER_ERR_NOMEM), "Out of memory",
                  "Nomem string");
    ASSERT_STR_EQ(fixer_result_string(FIXER_ERR_NOFIX), "No fixes to apply",
                  "Nofix string");
    ASSERT_STR_EQ(fixer_result_string(FIXER_ERR_VERIFY), "Verification failed",
                  "Verify string");
    ASSERT_STR_EQ(fixer_result_string((fixer_result_t)99), "Unknown error",
                  "Unknown string");
}
