...
- Real-time syntax highlighting with kind-aware token classification
  (including path *shape* x *kind* coloring)
...
Modes are **presets, not restrictions** (PHILOSOPHY S4): POSIX mode sets
...
(Scalar / List / Map) with predictive type-mismatch warnings. PHILOSOPHY S7
...
| No implicit list-to-string coercion (SEMANTICS S3) | Working |
